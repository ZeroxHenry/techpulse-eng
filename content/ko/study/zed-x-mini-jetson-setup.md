...
이 글은 그 한 달 동안 겪은 모든 걸 정리한 거다. 같은 삽질 안 하려면 읽어봐.
...
2. **`i2cdetect -y -r 9`이 진단 도구다.** 소프트웨어 깔기 전에 하드웨어 연결부터 확인해봐. 버스 9에 아무것도 안 나오면 SDK 깔아봤자 의미 없다.

3. **포럼 버전 추천 믿지 마.** Stereolabs가 공식 지원하는 최신 JetPack 깔아봐. 지금 기준으로는 6.2.1이다.
